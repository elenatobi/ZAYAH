...
// Helper functions to manage the active state of elements
function clear() {
...
class Tabs {
    constructor(mainTabsElement) {
        this.tabs = [];
        this.eTabActive = null;
        this.mainTabs = mainTabsElement;
    }
    ...
    addTab(tabName, tabNode, onTabActivate) {
        let newETab = this.getTab(tabNode);
        if (!newETab) {
            newETab = document.createElement("div");
            newETab.className = "tab";
            newETab.appendChild(create("div", "name", tabName));

            let eDel = create("div", "del", "X");
            newETab.draggable = true;
            newETab.appendChild(eDel);

            newETab.addEventListener("click", () => {
                deactivate(this.eTabActive);
                this.eTabActive = newETab;
                activate(newETab);
                onTabActivate();
            });

            eDel.addEventListener("click", (evt) => {
                evt.stopPropagation();
                this.__removeTab(newETab);
            });

            this.mainTabs.appendChild(newETab);
            this.tabs.push([tabNode, newETab]);
        }

        newETab.click();
    }

    __removeTab(tabElement) {
        let newETabActive = this.eTabActive === tabElement ? tabElement.nextSibling || tabElement.previousSibling : this.eTabActive;
        const index = this.tabs.findIndex(([name, eTab]) => eTab === tabElement);
        if (index > -1) {
...
class AuraBloodGemWrapper {
    constructor() {
        ...
        this.tabs = new Tabs(document.getElementById("mainTabs"));
    ...
    __createTree(node) {
        ...
        if (Array.isArray(subnodes)) {
            ...
            eNodeName.addEventListener("click", function() {
                sys.tabs.addTab(nodeName, node, () => sys.renderTabContent(nodeName, subnodes));
            });
        }
    ...
    renderTabContent(name, data) {
        let content = this.appeal(name, data);
        this.castVNode(content);
    }

    // Method to generate content for the tabs (using the appeal method)
    appeal(name, data) {
