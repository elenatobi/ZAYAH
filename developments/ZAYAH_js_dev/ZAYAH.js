...
function genMouse(name) {
    return function (event, hit) {
        ...
        if (target) {
            target.fire(name, new MouseEvent({ x: x, y: y }));
...
function genMouseOut(name) {
    return function (event, hit) {
        ...
        for (let color in colorMap) {
            ...
            if (node != target) {
                node.fire(name, new MouseEvent({ x: x, y: y }));
            }
        }
    };
}
...
const eventMap = {
    ...
    dragstart: [
        ...
        function (event, hit) {
            ...
            let target = colorMap[color];
            if (target) {
                target.fire("dragstart", new MouseEvent({ x: x, y: y }));
            }
            isDragging = true;
        },
    ],
    drag: [
        ...
        function (event, hit) {
            ...
            let target = colorMap[color];
            if (target) {
                target.fire("drag", new MouseEvent({ x: x, y: y }));
            }
        },
    ],
    dragend: [
        ...
        function (event, hit) {
            ...
            let target = colorMap[color];
            if (target) {
                target.fire("dragend", new MouseEvent({ x: x, y: y }));
            }
            isDragging = false;
        },
    ],
    touchstart: ["touchstart", function(event, hit){
        let rect = event.target.getBoundingClientRect();
        let dx = rect.left;
        let dy = rect.top;
        for (let touch of event.touches){
            let x = touch.clientX-dx;
            let y = touch.clientY-dy;
            let pixel = hit.getImageData(x, y, 1, 1).data;
            let color = `rgb(${pixel[0]}, ${pixel[1]}, ${pixel[2]})`;
            let target = colorMap[color];
            if (target) {
                target.fire("touchstart", new MouseEvent({ x: x, y: y }));
            }
        }
        touches = event.touches;
    }],
    touchmove: ["touchmove", function(event, hit){
        let rect = event.target.getBoundingClientRect();
        let dx = rect.left;
        let dy = rect.top;
        for (let touch of event.touches){
            ...
            let target = colorMap[color];
            if (target) {
                target.fire("touchmove", new MouseEvent({ x: x, y: y }));
            }
        }
        touches = event.touches;
    }],
    touchend: ["touchend", function(event, hit){
        if (!touches){
            return;
        }
        let rect = event.target.getBoundingClientRect();
        ...
        for (let touch of touches){
            ...
            let target = colorMap[color];
            if (target) {
                target.fire("touchend", new MouseEvent({ x: x, y: y }));
            }
        }
        touches = null;
    }],
