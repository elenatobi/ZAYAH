...
class Shape {
    ...
    render(ctx) {
        ...
        ctx.save();
        ctx.transform(...this.getTransform().getMatrix());
        this.__render(ctx);
        ctx.restore();
    ...
    getTransform() {
        ...
        return new Transform().translate(x, y).rotate(this.rotation).translate(-x, -y);
    }
...
class BoxShape extends Shape {
    constructor(x, y, width, height, color = "black", skewX = 0, skewY = 0) {
        ...
        this.__dim = new Transform([width, skewY, skewX, height, x, y]);
    }

    __render(ctx) {
        ctx.fillStyle = this.color;
        ctx.transform(...this.__dim.getMatrix());
    }

    transform(matrix) {
        this.__dim = new Transform().multiply(matrix).multiply(this.getTransform()).multiply(this.__dim);
        let t = this.getTransform().invert();
        this.__dim = t.multiply(this.__dim)
    }

    getBoundRect() {
        let R = this.getTransform().multiply(this.__dim);
        let P1 = R.point(0, 0);
        let P2 = R.point(0, 1);
        let P3 = R.point(1, 0);
        let P4 = R.point(1, 1)
        let xArr = [P1[0], P2[0], P3[0], P4[0]];
        let yArr = [P1[1], P2[1], P3[1], P4[1]];
        return [
            Math.min(...xArr),
            Math.min(...yArr),
            Math.max(...xArr),
    ...
    isInside(x, y) {
        [x, y] = this.getTransform().multiply(this.__dim).invert().point(x, y)
        return this.__isInside(x, y);
    }
    ...
    getPivot() {
        return this.__dim.point(0.5, 0.5);
    }
}
...
class Text extends BoxShape{
    constructor(x, y, width, height, color = "black", content, skewX = 0, skewY = 0){
        super(x, y, width, height, color, skewX, skewY);
    ...
    render(ctx){
        ...
        ctx.fillText(this.content, 0, 0, 1);
        ctx.beginPath();
        ctx.rect(0, 0, 1, 1);
        ctx.closePath();
        ctx.restore();
...
class EditText extends Text{
    constructor(x, y, width, height, color = "black", content, skewX = 0, skewY = 0){
        super(x, y, 0, height, color, content, skewX, skewY);
        this.maxWidth = width;
    }
    
    render(ctx){
        super.render(ctx);
        this.maxWidth = ctx.measureText(this.content).width
    }
}


let s = new Ellipse(80, 50, 200, 250, "rgba(255, 0, 0, 0.5)", 50, 25);
let p = new Text(10, 10, 100, 15, "yellow", "Hello world");
p.border = [1, "solid", "red"]
s.rotation = Math.PI / 3;
let [xLeft, yLeft, ,] = s.getBoundRect();
s.transform(new Transform([1, 0, 0, 1, xLeft, yLeft]).multiply(new Transform([1.3, 0, 0, 1, 0, 0])).multiply(new Transform([1, 0, 0, 1, -xLeft, -yLeft])));
...
p.mouse("move", function (x, y, event) {
    p.color = "green";
    renderFlag = true;
});

p.mouse("moveout", function(x, y, event){
    p.color =  "yellow";
    renderFlag = true;
});
